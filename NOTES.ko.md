...
**현재 단계: P6 완료 — 부팅 자동 로드와 배포 패키징까지 검증.**
실기(82547EI) 실측 172 Mbit/s 송신 / 248 Mbit/s 수신.
...
**오류 0, 충돌 0.** 드라이버 로그에 타임아웃·경고·실패가 한 건도 없다.
...
**이것은 NIC가 둘인 동안만의 문제다.** 82547EI가 온보드이므로 드라이버가
완성되면 tulip을 제거할 계획이고, 그러면 인터페이스가 하나가 되어
NetInfo가 en1을 정상 설정하게 된다.
...
- **git 최초 커밋.** 이 저장소는 아직 한 번도 커밋된 적이 없다.
  동작하는 드라이버가 작업 트리에만 존재한다 — 가장 위험한 미결 항목.
- 에러·링 고갈 복구. 링이 고갈되거나 에러 비트가 서는 경우의 복구
  경로가 없다.
- 링크 변동 대응(케이블 탈착). LSC 인터럽트는 받고 있으나 재협상
  경로를 검증하지 않았다.
- 82547EI 외 부품 검증. 계열 코드는 SDM·FreeBSD 기준으로 구현했을 뿐
  실기가 없다. 로그에 `[UNVERIFIED PART]`로 표시된다.
- tulip 제거 + `en1`의 NetInfo 정식 등록. 지금은 부팅 때 en1이 주소
  없이 올라와 잘못된 기본 경로를 만든다(위 "과도기 주의" 참조).
  부팅 시 `No response from network configuration server.` 지연도
  NIC이 둘인 탓이며 같이 해소될 것으로 본다.
