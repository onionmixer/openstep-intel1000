...
>
> **현재: P1~P6 전부 완료.** 정찰 → MAC/링크 → 수신 → 송신 →
> 안정화 → IONetwork 통합·부팅 자동 로드까지 실기 검증했다.
> 실측 172 Mbit/s 송신 / 248 Mbit/s 수신 (82547EI).
> 남은 것은 에러·링 고갈 복구, 링크 변동 대응, 82547EI 외 부품 검증.
...
| P5 | 안정화 — 멀티캐스트, 에러 복구, 부하 | 장시간 무결함 | 🔶 대부분 완료 (에러·링 고갈 복구 잔여) |
...
- **머신 하드 행 — 실제로 두 번 일어났다.** 예상은 "커널 패닉"이었으나
  ...
  · **`./tools/nx-logcatch.sh start`** — 커널 로그를 NFS로 밀어내
    ...
    한 번의 행으로 특정했다.
  · **한 번에 한 단계씩** — 드라이버의 `CALL` 인자가 단계 선택자다.
...
- **성능 기대치 관리**: PCI 32bit/33MHz + 구형 CPU라 기가비트
  ...
  성공으로 정의했고, **실측 172/248 Mbit/s로 충족했다**(tulip 대비 21배).
